...
export const bySubscription = catchAsyncError(async (req, res, next) => {
    ...
    if (user.role === 'admin') return next(new ErrorHandler('admin cant buy subcription', 400));

   const plan_id = process.env.PLAN_ID || "plan_LdcuefqpHSg3hD"
...
export const paymentVarification = catchAsyncError(async (req, res, next) => {
    ...
    res.redirect(`${process.env.FRONTEND_URL}/paymentsucess?refrence=${razorpay_payment_id}`)
});
...
//     res.status(200).json({
//         success: true,
//         message: refund ? 'subscription canclled, tou will recive full payment witin 7 days' : 'subscription canclled, no refund will recive'
//     })
// });